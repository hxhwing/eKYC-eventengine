...
**本章节将创建后续实验中，需要共用的一些资源，包括：**
 ...
 - **DynamoDB:**   在 Workshop 初始化过程中已经创建 ```UserAccount```
 - **S3 Bucket:**  在 Workshop 初始化过程中已经创建 ```s3-kyc-bucket-"AccountId"```
 ...
 - **Textract S3 Bucket:**   因为 Amazon Textract 服务暂不支持东京区域，所以需要在支持 Textract 的区域再创建一个 S3 Bucket，在进行 OCR 处理时，先将证件照复制到 Textract 所在 Region 的 S3 bucket 中。在本 Workshop 中，使用 ap-northeast-2 区域。
